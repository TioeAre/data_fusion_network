...
def cross_entropy2d(input, target, weight=None, size_average=True):
    ...
    # log_p = log_p.contiguous() # transpose(1, 2).transpose(2, 3).
    # log_p = log_p[target >= 0] # .long()  # Remove the unnecessary expansion of target

    # target = target[target >= 0].long()
    # loss = F.nll_loss(log_p, target, weight=weight, reduction='sum')
    ...
    #     loss /= mask.data.sum()
    # print(loss)
    # result = torch.sub(input, target)
    loss = F.binary_cross_entropy_with_logits(input, target, reduction='none').mean()
    print(loss)
    return loss
